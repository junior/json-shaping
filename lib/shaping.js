 ...
 * Expose `jsonShaping()`.
 */
exports = module.exports = jsonShaping;

/**
 * Shape json.
 * @param {json} data Data to be shaped
 * @param {json} map Template for result
 * @return {Function}
 * @api public
 */
function jsonShaping(data, map) {
    return {
        ...
        transform: function () {
            ...
            if (value) {
                ...
                normalized = map.item ? _.map(list, _.bind(this.iterator, this,
                    map.item)) : list;
            ...
            if (map.operate) {
                _.each(map.operate, _.bind(function (method) {
                    data = _.map(data, _.bind(function (item) {
                        ...
                        this.setValue(item, method.on, fn(this.getValue(item,
                            method.on)));
                        return item;
                    }, this));
                }, this));
        ...
        iterator: function (map, item) {
            ...
            _.each(map, _.bind(function (oldkey, newkey) {
                if (typeof oldkey === 'string' && oldkey.length > 0) {
                    ...
                    var array = _.map(oldkey, _.bind(function (item, map) {
